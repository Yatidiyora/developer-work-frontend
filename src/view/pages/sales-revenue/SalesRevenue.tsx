...
const subCategoryOptions: {
  [key in 'electronics' | 'clothing' | 'furniture']: { value: string; label: string }[];
} = {
  electronics: [
...
const SearchExpandableRow = (props: any) => {
              ...
              <FormikForm>
                <Row>
                  <Col style={{ marginBottom: '10px' }} md={3}>
                    <Form.Group>
                      <Form.Label className="default-filter-form__label">{'Category'}</Form.Label>
                      <Field
                        name="category"
                    ...
                    <Col md={2}>
                      <Form.Group>
                        ...
                        <Field
                          name="subCategory"
                          component={DefaultCustomSelect}
                          options={subCategoryOptions[values.category.toLowerCase() as keyof typeof subCategoryOptions]}
...
const SalesRevenue = () => {
  ...
  useEffect(() => {
    const searchRevenue = async () => {
      ...
      const rangeValue: { categoryType: any; rangeValue?: any } = { categoryType: searchValues?.dateType };
      ...
      if (
        ...
        rangeValue['rangeValue'] = [searchValues['startDate'], searchValues['endDate']];
      }
      const dateCategory = {
        ...rangeValue,
      };
  ...
  return (
    <div className="containt-management">
      <div className="containt-management-header">
        <SearchExpandableRow title={'Search Sales Revenue'} setSearchValues={setSearchValues} />
      </div>
      <div className="graph-container">
        <div id={CHART_ROOT_TAGS.SALES_REVENUE_TAG} style={{ width: '100%', height: '300px' }} />
      </div>
    </div>
  );
};
