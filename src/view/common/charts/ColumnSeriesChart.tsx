...
const simpleColumnSeriesChart = (
  displayData: any,
  rootTag: string,
  xCategoryField: string,
  seriesProps: ColumnSeriesProps[],
  xLabel?: string
) => {
  ...
  const seriesCreator = (props: ColumnSeriesProps) => {
    const {
      fontSize,
      opposite,
      gridVisible,
  ...
  marginSeries.forEach((series, seriesIndex) => {
    series.columns.template.adapters.add("layer", (layer, target) => {
      ...
      const columnPositions = marginSeries.map((s) => {
      ...
      // console.log(xCategory, columnPositions);
...
export { simpleColumnSeriesChart };
