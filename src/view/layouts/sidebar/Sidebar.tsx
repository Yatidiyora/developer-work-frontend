...
import { SidebarConfig, SidebarType } from '../../../common/types/interface/Layouts.interface';

const Sidebar = ({ collapse, setCollapse, config, openDropdowns, toggleDropdown }: SidebarType) => {
  const initialRenderPath = '/' + window.location.pathname.split('/').filter((path) => path)[0];
          ...
          >
            ...
            {!collapse && (
              <button style={{ background: 'none', border: 'none' }} type="button">
                ...
                <span
                  className={`ms-2 ${
                    isOpen ? 'bi bi-chevron-compact-up' : 'bi bi-chevron-compact-down'
                  }`}
                  style={{
                    color: '#33FFEC',
                  }}
            ...
            {item.children.map((child, index) => {
              ...
              return (
                <div key={index}>
                  ...
                  <li className={`sidebar-subitem ${activePath === child.path ? 'active' : ''}`}>
                    <Link
  ...
  return (
    ...
    >
        ...
        >
          <button
            ...
            style={{
              ...
              background: '#00E5FF',
              ...(!collapse ? { borderTopLeftRadius: '6px' } : { borderTopRightRadius: '6px' }),
              ...(!collapse
      ...
      <nav>
        <ul className="sidebar-menu">{config.map((item) => renderMenuItem(item))}</ul>
      </nav>
    </div>
  );
};
